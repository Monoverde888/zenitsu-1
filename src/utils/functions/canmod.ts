...
function canMod(member: detritus.Structures.Member, client: detritus.ShardClient, mod: 'kick' | 'ban' | null): boolean {
  ...
  return (roleSort.position > memberRoleSort.position) && (mod ? (hasPermission(Number(member.guild.me.permissions), mod == 'kick' ? Flags.KICK_MEMBERS : Flags.BAN_MEMBERS)) : true);
}
