...
export default class Comando extends Command {
  ...
  async run({ message, langjson }: run): Promise<light.Message> {
    ...
    return message.channel.createMessage({ content: 'Discord Status https://discordstatus.com/' }, { file: att, name: 'img.png' }).finally(async () => {
      return await msg.delete().catch(() => undefined);
    })
